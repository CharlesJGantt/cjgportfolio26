...
export const WebsitesSection = () => {
  const { work: sites, sectionTitle, sectionDescription } = DATA.websites;

  const [selectedSite, setSelectedSite] = useState<Project | null>(null);

  const handleOpenModal = (site: Project) => setSelectedSite(site);
  const handleCloseModal = () => setSelectedSite(null);

  return (
    <section className="py-20 bg-background" id="websites-section">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 md:gap-6">
          {sites.slice(0, 3).map((site, index) => (
            ...
            >
              <ProjectCard
                project={site}
                onViewDetails={() => handleOpenModal(site)}
              />
            </motion.div>
          ))}
        </div>
